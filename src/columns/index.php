...
class Columns extends PRC_Core_Block_Library {
	...
	public function __construct($init = false) {
		if ( true === $init ) {
			$block_json_file = PRC_CORE_BLOCK_LIBRARY_DIR . '/build/columns/block.json';
	...
	public function add_attributes( $metadata ) {
		...
		if ( ! array_key_exists( 'fillViewportHeight', $metadata['attributes'] ) ) {
			$metadata['attributes']['fillViewportHeight'] = array(
				'type'    => 'boolean',
				'default' => false
			);
		}
	...
	/**
	 * Register additional settings for columns block.
	 * - Add `fillViewportHeight` to block context.
	 *
	...
	public function add_settings(array $settings, array $metadata) {
		if ( self::$block_name === $metadata['name'] ) {
			$settings['provides_context'] = array_merge(
				...
				array(
					'core/column/fillViewportHeight' => 'fillViewportHeight'
				)
			);
		}
		return $settings;
